...
async function fillBlanks() {
  let lastUpdated = await browser.storage.sync.get({ last_update: "" });
  let statusObj = await browser.storage.sync.get({ status: "operational" });
  ...
  let others = await browser.storage.sync.get({ othersDown: false });
